from encryption import BLOCK_SIZE, generate_keys
from utility import xor


S_box = []


def step(data: bytearray, key: bytearray) -> bytearray:
    s1 = xor(data, key)
    s2 = bytearray(s1)
    s2[2] = s1[3]
    s2[3] = s1[2]
    s3 = bytearray()
    for byte in s2:
        s3.append(S_box[byte])
    return s3


def AES(data: bytearray, keys: list, dbg) -> bytearray:
    r2 = step(data, keys[2])
    if dbg:
        print("Round 0: ", r2.hex())
    r1 = step(r2, keys[1])
    if dbg:
        print("Round 1: ", r1.hex())
    r0 = xor(r1, keys[0])
    if dbg:
        print("Result: ", r0.hex())
    return r0


def ECB(data: list, key: list, dbg) -> bytearray:
    plain_text = bytearray()
    i = 0
    for chunk in data:
        i += 1
        if dbg:
            print("Block {0}: {1}".format(i, chunk.hex()))
        for byte in AES(chunk, key, dbg):
            plain_text.append(byte)
    return plain_text


def CBC(data: list, key: list, dbg) -> bytearray:
    plain_text = bytearray()
    plain_blocks = []



def decrypt(data: bytearray, key: bytearray, mode: str, dbg=False, iv=None) -> bytearray:
    if len(key) != BLOCK_SIZE:
        raise Exception("Incorrect size of key!")
    data_chunks = [data[i: i + BLOCK_SIZE] for i in range(0, len(data), BLOCK_SIZE)]
    k = generate_keys(key)
    if dbg:
        print("K0: ", k[0].hex())
        print("K1: ", k[1].hex())
        print("K2: ", k[2].hex())
    if mode == 'ecb':
        return ECB(data_chunks, k, dbg)
